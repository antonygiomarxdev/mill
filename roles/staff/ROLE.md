...
Before declaring merge-ready, verify:

1. `git log` — commits exist, are incremental, count matches brief
2. `git diff --stat` — only files in the brief were touched
3. `reviewed_by` approved (check issue labels)
4. Reviewer gave `APPROVED` (check issue labels)
5. Recalculate every quantitative claim in the agent's report. Run the grep the agent claims to have run.
6. Execute mechanical checks from the brief. Verify output matches expected.
7. Gates pass: lint, type-check, build

### CTO + PM collaboration

- **CTO ↔ Staff**: technical direction, architecture, merge approval. Direct.
- **CTO ↔ PM**: product decisions, scope, priorities. Direct.
- **Staff ↔ PM**: Staff researches viability, PM refines specs. Close collaboration.
- **PM ↔ UX/UI**: PM passes product specs to design.
- **Staff ↔ Tech Lead**: Staff delegates technical execution.

When the CTO session needs PM input, you spawn PM as a subagent with a concrete question. PM answers in its own worktree. You bring the output to the CTO. The CTO never loads two roles.
