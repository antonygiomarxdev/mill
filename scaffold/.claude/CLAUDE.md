...
After every dispatch, verify the worker's output in its worktree:
`.mill/checks/mill-verify --worktree <path> --role <role> [--files-modified
<list>]` runs the gauntlet (build/lint/test) and enforces the role's file
permissions over the change set. What "build" and "test" mean is per project —
Mill ships no language-specific tooling of its own.
